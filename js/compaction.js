...
// TEST CONTROL
// ================================================================
async function startTest(){var connType=document.getElementById('com-port-select').value;if(!connType){alert('Select connection first');return;}hammerWeight=parseFloat(document.getElementById('inp_hammer_weight').value)||2.5;moldVolume=parseFloat(document.getElementById('inp_mold_volume').value)||0.001;targetStrikes=parseInt(document.getElementById('inp_target_strikes').value)||25;targetRatio=parseFloat(document.getElementById('inp_target_ratio').value)||95;document.getElementById('target-display').textContent=targetStrikes;strikes=[];isTesting=true;isDemoMode=(connType==='demo');document.getElementById('btn-start').style.display='none';document.getElementById('btn-stop').style.display='flex';document.getElementById('btn-tare').style.display='flex';document.getElementById('live-indicator').style.display='flex';document.getElementById('results-panel').style.display='none';document.getElementById('btn-pdf').style.display='none';document.getElementById('strike-log-body').innerHTML='';logTestStarted('compaction',currentTest?currentTest.id:'');if(isDemoMode){document.getElementById('demo-banner').style.display='flex';document.getElementById('serial-dot').className='status-dot scanning';document.getElementById('serial-text').textContent='Demo';runDemoMode();}else{document.getElementById('demo-banner').style.display='none';if(!serialPort){var ok=await connectSerial();if(!ok){stopTest();return;}}await sendSerialCommand('START');startSerialStream();}}
function stopTest(){isTesting=false;document.getElementById('btn-start').style.display='flex';document.getElementById('btn-stop').style.display='none';document.getElementById('live-indicator').style.display='none';if(!isDemoMode){sendSerialCommand('STOP');stopSerial();}if(strikes.length>0){logTestStopped('compaction',currentTest?currentTest.id:'',{strikeCount:strikes.length});calculateFinalResults();}else{logTestStopped('compaction',currentTest?currentTest.id:'',{strikeCount:0,abandoned:true});}}
...
db.collection('sessions').add({testId:currentTest.id,domainId:currentDomain.id,domainName:currentDomain.name,testName:currentTest.name,engineerInputs:{hammer_weight:hammerWeight,mold_volume:moldVolume,target_strikes:targetStrikes,target_ratio:targetRatio,ref_weight:rw},strikes:strikes,results:results,userId:currentUser?currentUser.uid:'guest',createdAt:firebase.firestore.FieldValue.serverTimestamp()}).then(function(docRef){currentSessionId=docRef.id;logSessionCreated(docRef.id,'compaction');displayResults(results);loadHistory();});}

function displayResults(results){var panel=document.getElementById('results-panel');var body=document.getElementById('results-body');panel.style.display='block';var labels={strikes_count:'Strikes Completed',target_strikes:'Target Strikes',max_dry_density:'Max Dry Density (kg/m³)',optimum_moisture:'Optimum Moisture %',average_moisture:'Avg Moisture %',average_force:'Avg Force (N)',max_force:'Max Force (N)',reference_density:'Reference Density (kg/m³)',compaction_ratio:'Compaction Ratio %',hammer_weight:'Hammer (kg)',mold_volume:'Mold (m³)',status:'Status'};var html='';if(results.status){var p=results.status==='PASS';html+='<div class="result-status '+(p?'pass':'fail')+'">'+(p?'✅':'❌')+' '+results.status+' — '+results.compaction_ratio+'%</div>';}Object.keys(results).forEach(function(k){html+='<div class="result-row"><span class="result-label">'+(labels[k]||k)+'</span><span class="result-value">'+results[k]+'</span></div>';});body.innerHTML=html;if(results.compaction_ratio>0){document.getElementById('chart-box').style.display='block';setTimeout(function(){document.getElementById('chart-fill').style.width=Math.min(results.compaction_ratio,100)+'%';},100);}document.getElementById('btn-pdf').style.display='flex';var mlBtn=document.getElementById('btn-ml-compaction');if(mlBtn)mlBtn.style.display='flex';}

async function loadHistory(){if(!currentTest)return;try{var snap=await db.collection('sessions').where('testId','==',currentTest.id).get();var tbody=document.getElementById('history-body');if(snap.empty)return;tbody.innerHTML='';var rows=[];snap.forEach(function(doc){var s=doc.data();rows.push(s);});rows.sort(function(a,b){return (b.createdAt&&b.createdAt.seconds||0)-(a.createdAt&&a.createdAt.seconds||0);});rows.slice(0,10).forEach(function(s){var st=(s.results&&s.results.status)||'?';var dens=(s.results&&s.results.max_dry_density)||'--';var date=s.createdAt?new Date(s.createdAt.seconds*1000).toLocaleDateString():'--';var cls=st==='PASS'?'result-pass':(st==='FAIL'?'result-fail':'');var tr=document.createElement('tr');tr.innerHTML='<td>'+date+'</td><td class="'+cls+'">'+st+'</td><td>'+dens+' kg/m³</td>';tbody.appendChild(tr);});}catch(e){console.error('loadHistory error:',e);showToast('Failed to load history: '+e.message,'error');}}
