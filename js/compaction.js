...
function runDemoMode(){var n=0;var calibData=[{moisture:8.2,force:210},{moisture:10.5,force:285},{moisture:12.8,force:340},{moisture:14.2,force:365},{moisture:15.1,force:355},{moisture:16.8,force:330},{moisture:18.5,force:290},{moisture:20.2,force:245}];var iv=setInterval(function(){if(!isTesting||n>=Math.min(targetStrikes,calibData.length)){clearInterval(iv);if(isTesting)stopTest();return;}var d=calibData[n];processStrike(d.moisture,d.force);n++;},800);}
...
function processStrike(moisture, force) {
    ...
    var wetDensity = typeof calcWetDensity === 'function' ? calcWetDensity(force, moldVolume) : force / (moldVolume * 9.81);
    var dryDensity = typeof calcDryDensity === 'function' ? calcDryDensity(wetDensity, moisture) : wetDensity / (1 + moisture / 100);
    var strike = {
...
// FINAL RESULTS
// ================================================================
function calculateFinalResults(){if(strikes.length===0)return;var densities=strikes.map(function(s){return{dryDensity:s.dryDensity,moisture:s.moisture,force:s.force};});var proctor=typeof proctorMaxDryDensity==='function'?proctorMaxDryDensity(densities):{};var mdd=proctor.maxDryDensity||0,om=proctor.optimumMoisture||0;var tm=0,tf=0,mf=0;for(var i=0;i<strikes.length;i++){tm+=strikes[i].moisture;tf+=strikes[i].force;if(strikes[i].force>mf)mf=strikes[i].force;}var am=tm/strikes.length,af=tf/strikes.length;var rw=parseFloat(document.getElementById('inp_ref_weight').value)||0;var rd=rw>0?rw/moldVolume:0;var cr=typeof calcCompactionRatio==='function'?calcCompactionRatio(mdd,rd):(rd>0?(mdd/rd)*100:0);var passed=cr>=targetRatio;var results={strikes_count:strikes.length,target_strikes:targetStrikes,max_dry_density:Math.round(mdd*100)/100,optimum_moisture:Math.round(om*100)/100,average_moisture:Math.round(am*100)/100,average_force:Math.round(af*100)/100,max_force:Math.round(mf*100)/100,reference_density:Math.round(rd*100)/100,compaction_ratio:Math.round(cr*100)/100,hammer_weight:hammerWeight,mold_volume:moldVolume,status:passed?'PASS':'FAIL'};
