...
function serialLog(text,type){var con=document.getElementById('serial-console');if(con.style.display==='none')con.style.display='block';var now=new Date().toLocaleTimeString('en-US');var cls=type==='rx'?'rx':(type==='tx'?'tx':'');var div=document.createElement('div');div.className='serial-line';var tsSpan=document.createElement('span');tsSpan.className='ts';tsSpan.textContent='['+now+']';var msgSpan=document.createElement('span');if(cls)msgSpan.className=cls;msgSpan.textContent=sanitizeInput(text)||'';div.appendChild(tsSpan);div.appendChild(msgSpan);con.appendChild(div);con.scrollTop=con.scrollHeight;}
